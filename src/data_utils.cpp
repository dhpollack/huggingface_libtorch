...
torch::data::Example<> SST2::get(size_t index) {
    ...
    std::vector<int64_t> label_vec{p.second};
    torch::Tensor label = torch::from_blob(label_vec.data(), {1});
    std::vector<int> token_ids_raw;
    processor_->Encode(p.first, &token_ids_raw);
    torch::Tensor token_ids = torch::from_blob(token_ids_raw.data(), {token_ids_raw.size()});
...
// dataset text to token_ids
void SST2::t2id(std::string& s, sentencepiece::SentencePieceProcessor& sp) {
