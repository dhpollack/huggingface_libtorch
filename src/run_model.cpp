...
int main(int argc, char* argv[]) {
	...
	cout << "   i.e. `./huggingface-albert ../models/sst2_trained ../data/SST-2/dev.tsv`" << endl;
	return -1;
    }
    ...
    auto dl = torch::data::make_data_loader<torch::data::samplers::SequentialSampler>(move(ds), BATCH_SIZE);
    ...
    for (auto& mb : *dl) {
	cout << "Batch Size: " << mb.data.size(0) << ", " << mb.data.size(1) << endl;
