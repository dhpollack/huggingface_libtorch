...
template <typename InputID = Tensor, typename AttentionMask = Tensor,
          typename TokenTypeID = Tensor, typename PositionID = Tensor,
...
struct TransformerExample {
  ...
  TransformerExample(InputID input_ids, AttentionMask attention_mask,
      ...
      : input_ids(std::move(input_ids)),
        ...
        position_ids(std::move(position_ids)), target(std::move(target)) {}
